...
def run():
    with tempfile.TemporaryDirectory() as tmp:
        bridge = EliteTokenBridge(EliteMemoryCache(tmp)); results = {}
        for name, prompt in WORKLOADS.items():
            reduced = bridge.compress_context(prompt, compression_ratio=0.1)
            results[name] = {"input_bytes": len(prompt.encode()), "output_bytes": len(reduced.encode()), "input_lines": len(prompt.splitlines()), "output_lines": len(reduced.splitlines())}
        return {"workloads": results, "measurement": "fixture byte and line counts only"}
